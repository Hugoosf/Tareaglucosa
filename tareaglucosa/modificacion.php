...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $sql_controlglu = "UPDATE controlglu SET lenta = ?, deporte = ? WHERE fecha = ? AND idusuario = ?";
        $stmt_controlglu = $conn->prepare($sql_controlglu);
        $stmt_controlglu->bind_param("sssi", $lenta, $deporte, $fecha, $idusuario);
        ...
        foreach ($comidas as $comida) {
            ...
            $sql_comidas = "UPDATE comidas SET gl1h = ?, rac = ?, insu = ?, glh2 = ? WHERE fecha = ? AND idcomida = ? AND idusuario = ?";
            $stmt_comidas = $conn->prepare($sql_comidas);
            $stmt_comidas->bind_param("ssssssi", $gl1h, $rac, $insu, $gl2h, $fecha, $comida, $idusuario);
            ...
            if (isset($_POST[$comida . '_hipo_glu'], $_POST[$comida . '_hipo_hora'])) {
                ...
                $sql_hipo = "UPDATE hipo SET glu = ?, hora = ? WHERE fecha = ? AND idcomida = ? AND idusuario = ?";
            ...
            if (isset($_POST[$comida . '_hiper_glu'], $_POST[$comida . '_hiper_hora'], $_POST[$comida . '_hiper_corr'])) {
                ...
                $sql_hiper = "UPDATE hiper SET glu = ?, hora = ?, corr = ? WHERE fecha = ? AND idcomida = ? AND idusuario = ?";
...
<body>
    <h1>Secciona la fecha del registro modifícalo</h1>
